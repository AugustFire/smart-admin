...
### 后端 (auto-admin-server)

```bash
cd auto-admin-server && mvn spring-boot:run    # 启动后端
...
```bash
cd auto-admin-web && pnpm dev    # 启动开发服务器
pnpm build                        # 构建生产版本
pnpm preview                      # 预览构建结果
pnpm lint                         # 代码检查
```
...
```bash
mysql -u root -p auto_admin < sql/schema.sql    # 执行表结构脚本
mysql -u root -p auto_admin < sql/data.sql      # 执行初始化数据脚本
```
...
```bash
mysqlsh -u root -p123456 --sql -D auto_admin < your-script.sql
mysqlsh -u root -p123456 --sql -D auto_admin -e "SHOW TABLES;"
mysqlsh -u root -p123456 --sql -D auto_admin -e "SELECT * FROM sys_api;"
```